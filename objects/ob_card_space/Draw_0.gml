if trash=true {
	...
	if instance_exists(ob_event) {
		...
		if ob_event.event_kind=ref_event_levelup {
			...
			if occupy_id!=-1 and occupy_id.card_level<ob_main.card_level_player_limit and occupy_id.card_innate>0 and ob_event.event_applied=false {
				if ob_main.money>=ob_main.event_cost_standby { var money_color=global.color_card_light; }
				else { var money_color=global.color_damage; }
				sc_drawtext(x+28,y-34,"Cost: $" + string(ob_main.event_cost_standby) + " / $" + string(ob_main.money),money_color,global.color_black,1,1,0,-1);
				//
				with (occupy_id) {
				...
				//
				var levelup_info_text="+" + string(atk_diff) + " ATK / +" + string(def_diff) + " DEF / +" + string(hp_diff) + " HP";
