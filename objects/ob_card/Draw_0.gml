...
if card_shiny = true
{
	...
	if reference_id=ob_control 
	{
		...
		//else if (card_glyph_a = ref_glyph_rush or card_glyph_b = ref_glyph_rush or card_glyph_c = ref_glyph_rush) && ob_control.turn_num <= 2 { var base_atk_multiplier=1.5; }
			...
			if card_face = true {
				...
				if (card_glyph_a = ref_glyph_rush or card_glyph_b = ref_glyph_rush or card_glyph_c = ref_glyph_rush) && ob_control.turn_num <= 2
				{
					card_bonus_atk += ceil((ob_control.card_space_id[i].occupy_id.card_atk*1.5)-ob_control.card_space_id[i].occupy_id.card_atk);
				}
			}
	}
	else
