...
//————————————————————————————————————————————————————————————————————————————————————————————————————
// DRAW: AT LEAST 1 POKEMON > ANOTHER POKEMON IF ONLY 1 POKEMON AND 4+ BERRIES (NOT ENIGMA) > BERRIES IF NEEDED (NOT ENIGMA) > POKEMON > BERRIES
//————————————————————————————————————————————————————————————————————————————————————————————————————
else if random_value<100 and ob_main.playing_tutorial=false {
	if poke_held<1 and AI_draw_available(0) {
		AI_draw(0);
	}
	else if poke_held=1 and (berries_held[0]+berries_held[1]+berries_held[2])>=4 and AI_draw_available(0) {
		AI_draw(0);
	}
