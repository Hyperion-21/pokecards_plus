...
if argument1=false {
	switch (argument0) {
		...
		case ref_glyph_harvest: return		"// HARVEST //\nConsumed Berries return to hand (if not full)\nwhen fainting."; break;
		...
		case ref_glyph_counter: return		"// COUNTERATTACK //\nWhen attacked, the attacker is also hurt\n(without fainting)."; break;
		...
		case ref_glyph_tenacity: return		"// TENACITY //\nWhen fainting, it has a 50% chance to return\nto hand (if not full)."; break;
		case ref_glyph_bulwark: return		"// BULWARK //\nSummons rocks on each empty side when played."; break;
		case ref_glyph_fork: return			"// FORK ATTACK //\nAttacks diagonally to its sides (twice) instead\nof straight ahead,but base attack is reduced by 1/3."; break;
		case ref_glyph_vampire: return		"// VAMPIRE //\nWhen attacking another card, half of the damage\nis absorbed."; break;
	...
	switch (argument0) {
		...
		case ref_glyph_vampire: return 3; break;
		case ref_glyph_harvest: return 3; break;
		case ref_glyph_counter: return 3; break;
		...
		case ref_glyph_tenacity: return 3; break;
		default: return 2;
	}
}
//————————————————————————————————————————————————————————————————————————————————————————————————————
